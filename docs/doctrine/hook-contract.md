...
**Corrupt or missing `dev-state.json`.** If `State.load()` raises `StateError` (e.g. JSON decode failure, schema version unrecognised), the hook prints an `[ERROR by dev-rules]` message to stderr and exits with code `1` (not `2`). Exit code 1 causes Claude Code to display a warning without blocking the tool call. This avoids the failure mode where a corrupted state file permanently locks all tool use. Recovery: delete `.claude/dev-state.json` and re-run the workflow from `idle`.
