"""Phase 2.4 — src/claude_workflow/_templates/.claude/ is the source-of-truth
for what init-fresh.sh ships to fork users.

Verifies:
  - src/claude_workflow/_templates/.claude/settings.json exists and uses new entry-point form
  - src/claude_workflow/_templates/.claude/dev-rules.config.yaml exists and parses
  - src/claude_workflow/_templates/.claude/scripts/notify.sh exists and is executable
  - src/claude_workflow/_templates/.claude/settings.json hook commands match the live
...
def test_templates_settings_matches_live() -> None:
    """src/claude_workflow/_templates/.claude/settings.json equals .claude/settings.json.
    ...
    assert live == tpl, "src/claude_workflow/_templates/.claude/settings.json drifted from .claude/settings.json"


def test_templates_dev_rules_yaml_matches_live() -> None:
    """src/claude_workflow/_templates/.claude/dev-rules.config.yaml equals the live copy."""
    live = (REPO_ROOT / ".claude" / "dev-rules.config.yaml").read_text()
    tpl = (TPL / "dev-rules.config.yaml").read_text()
    assert live == tpl, (
        "src/claude_workflow/_templates/.claude/dev-rules.config.yaml drifted from "
        ".claude/dev-rules.config.yaml"
    )


def test_templates_notify_sh_matches_live() -> None:
    """src/claude_workflow/_templates/.claude/scripts/notify.sh equals the live copy."""
    live = (REPO_ROOT / ".claude" / "scripts" / "notify.sh").read_text()
    tpl = (TPL / "scripts" / "notify.sh").read_text()
    assert live == tpl, (
        "src/claude_workflow/_templates/.claude/scripts/notify.sh drifted from "
        ".claude/scripts/notify.sh"
    )


def test_templates_skills_match_live() -> None:
    """Every src/claude_workflow/_templates/.claude/skills/<name>/SKILL.md has a byte-identical live copy.

    Closes the drift gap surfaced by issue #13's review: the framework
    ships switch-mode-* skills under src/claude_workflow/_templates/.claude/skills/ as part of
    ...
    to _templates/ must also exist in dogfood with identical content.
    """
    tpl_skills = TPL / "skills"
    assert tpl_skills.is_dir(), "src/claude_workflow/_templates/.claude/skills/ missing"
    ...
    for tpl_skill_md in sorted(tpl_skills.rglob("SKILL.md")):
        ...
        assert live_skill_md.is_file(), (
            f"src/claude_workflow/_templates/.claude/skills/{rel} has no live counterpart at "
            f".claude/skills/{rel}"
        )
        assert live_skill_md.read_bytes() == tpl_skill_md.read_bytes(), (
            f".claude/skills/{rel} drifted from src/claude_workflow/_templates/.claude/skills/{rel}"
        )
